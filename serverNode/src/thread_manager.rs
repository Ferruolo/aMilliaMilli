...
use crate::internal_lang::{execute, OperationsLang};
use crate::thread_manager::ThreadSignal::*;

//TODO: Write unit tests
#[derive(Clone)]
enum ThreadSignal<T: Clone + Send + Display> {
...
struct DBWorker<T: Clone + Send + Display> {
    id: usize,
    join_handle: JoinHandle<()>,
    sender: Sender<ThreadSignal<T>>,
}

pub struct ThreadManager<T: Clone + Send + Display + 'static> {
    num_threads: usize,
...
impl<T: Clone + Send + 'static + Display> DBWorker<T> {
...
impl<T: Clone + Send + Display> ThreadManager<T> {
    pub(crate) fn new(n_threads: usize, mut db_reference: &mut Arc<Mutex<DataManager<T>>>) -> ThreadManager<T> {
        ...
        let master_thread = thread::spawn(move || {
            ...
            loop {
                match master_recv.recv_timeout(Duration::from_millis(5)).unwrap_or(Null) {
                    Task(t) => {
                        //NOTE: Maintain FIFO order
